...
public class DeviceService {
    ...
    public DeviceSummaryDTO updateStatus(Long customerId, Long deviceId, DeviceStatus newStatus) {
        ...
        // Se estiver tentando ativar e o device ainda não estava ACTIVE, valida limite
        if (newStatus == DeviceStatus.ACTIVE && device.getStatus() != DeviceStatus.ACTIVE) {
            int active = deviceRepository.countByCustomerIdAndStatus(customerId, DeviceStatus.ACTIVE);
            Customer ensuredOwner = Objects.requireNonNull(owner, "owner");
            int max = devicePolicyService.getAllowedDevices(customerId, ensuredOwner.getOfferType());
            if (active >= max) {
